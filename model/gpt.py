...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.p_embedding = nn.Embedding(context_length, model_dim)
       ...
       # Positional embedding lookup table (maps positions 0 to max_seq_len - 1)
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        embeddings = self.embedding(context) + self.p_embedding(torch.arange(self.context_length))

        
        h1 = self.model(embeddings)
    ...
    class TransformerBlock(nn.Module):
        ...
        class MultiHeadedSelfAttention(nn.Module):
            ...
            class SingleHeadAttention(nn.Module):
                ...
                def forward(self, embedded: TensorType[float]) -> TensorType[float]:
                    ...
                    scores = scores / (attention_dim ** 0.5) # Normalizing the scores produced because they can get real bgi

                    lower_triangular = torch.tril(torch.ones(context_length, context_length))
                    mask = lower_triangular == 0
                    scores = scores.masked_fill(mask, float('-inf'))
                    scores = nn.functional.softmax(scores, dim = 2) # Causal masking
        ...
        def forward(self, embedded: TensorType[float]) -> TensorType[float]:
            torch.manual_seed(0)
            embedded = embedded + self.attention(self.first_norm(embedded)) # skip connection (Residual Connections)
            embedded = embedded + self.linear_network(self.second_norm(embedded)) # another skip connection(Residual Connections)
            return embedded
